...
class PendingDraftController extends Controller
{
    ...
    /**
     * Export the current tab's leads as a plain printable corporate report.
    ...
    public function export(Request $request)
    {
        ...
        if ($tab === 'not_paid') {
            $leads = $query->whereNotNull('not_paid_at')->orderBy('not_paid_at', 'desc')->get();
        } else {
            $leads = $query->whereNull('not_paid_at')->orderBy('followup_done_at', 'desc')->get();
        }

        // Per-state sales count (sorted highest first, blanks last)
        $stateCounts = $leads
            ->groupBy(fn ($l) => strtoupper(trim($l->state ?: '')) ?: 'Unknown')
            ->map->count()
            ->sortDesc();

        $tabLabel   = $tab === 'not_paid' ? 'Not Paid / FDFP' : 'Awaiting Draft';
        $exportedAt = now()->format('F j, Y  g:i A');
        $fdfpTypes  = \App\Support\Statuses::FDFP_TYPES;

        return view('admin.pending-draft.export', compact(
            'leads', 'stateCounts', 'tab', 'tabLabel',
            'dateFrom', 'dateTo', 'search', 'exportedAt', 'fdfpTypes'
        ));
    }
