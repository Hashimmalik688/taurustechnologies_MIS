...
class ValidatorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $homeOfficeLeads = Lead::where('team', Teams::PEREGRINE)
            ->where('assigned_validator_id', Auth::id())
        ...
        $allLeads = Lead::where('team', Teams::PEREGRINE)
            ->where('assigned_validator_id', Auth::id())
            ...
            ->where(function($query) use ($startDate, $endDate) {
                ...
                ->orWhere(function($q) use ($startDate, $endDate) {
                    $q->whereIn('status', [Statuses::LEAD_SALE, Statuses::LEAD_DECLINED, Statuses::LEAD_RETURNED])
                      ->whereBetween('validated_at', [$startDate, $endDate]);
                });
            })
            ->get();

        $salesLeads = $allLeads->where('status', Statuses::LEAD_SALE);
        $declinedLeads = $allLeads->where('status', Statuses::LEAD_DECLINED);
        ...
        $submittedLeads = Lead::where('team', Teams::PEREGRINE)
            ->where('assigned_validator_id', Auth::id())
    ...
    public function returnToCloser(Request $request, $id)
    {
        ...
        $updateData['status'] = Statuses::LEAD_RETURNED;
