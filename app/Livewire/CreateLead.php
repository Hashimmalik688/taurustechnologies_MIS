...
class CreateLead extends Component
{
    ...
    protected $rules = [
        ...
        'beneficiaries.*.dob' => 'nullable|date|before:today',
    ...
    protected $messages = [
        ...
        'beneficiaries.*.name.required' => 'Beneficiary name is required.',
    ...
    {
        $this->beneficiaries[] = ['name' => '', 'dob' => '', 'relation' => ''];
    }

    public function removeBeneficiary($index)
    {
        if (count($this->beneficiaries) > 1) {
            unset($this->beneficiaries[$index]);
            $this->beneficiaries = array_values($this->beneficiaries); // Re-index array
        }
    }
    ...
    {
        // Filter out empty beneficiaries before validation
        $this->beneficiaries = array_filter($this->beneficiaries, function($b) {
            return !empty($b['name']);
        });
        $this->beneficiaries = array_values($this->beneficiaries); // Re-index
        ...
        // Store beneficiaries in JSON field if provided
        if (!empty($this->beneficiaries)) {
            $leadData['beneficiaries'] = $this->beneficiaries;
            // Also set the first beneficiary in the old field for backwards compatibility
            if (!empty($this->beneficiaries[0]['name'])) {
                $leadData['beneficiary'] = $this->beneficiaries[0]['name'];
                $leadData['beneficiary_dob'] = $this->beneficiaries[0]['dob'] ?? null;
            }
        }
        ...
        // Status 'closed' means the lead has been processed and should show in the main leads list
        $leadData['status'] = 'closed';
        $leadData['closer_name'] = auth()->user()->name ?? 'Manual Entry';
        $leadData['verified_by'] = null; // Keep null so it's not counted as a verifier submission

        // Convert boolean smoker to enum 'yes'/'no' for database
