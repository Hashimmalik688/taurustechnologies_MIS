...
<body>
    ...
    <div class="print-buttons no-print">
        ...
        <a href="{{ route('admin.accounting.journal.show', $entry->id) }}" class="btn-back">← Back</a>
    </div>
    ...
    <table class="meta-table">
        <tr>
            <td style="width:20%">
                ...
                <span class="meta-value">{{ $entry->entry_number }}</span>
            </td>
            <td style="width:18%">
                <span class="meta-label">Date</span>
                <span class="meta-value">{{ $entry->entry_date->format('d M Y') }}</span>
            </td>
            <td style="width:17%">
                ...
                <span class="meta-value">
                    ...
                    @endphp
                    <span class="badge {{ $cls }}">{{ $entry->type_label }}</span>
                </span>
            </td>
            <td style="width:20%">
                <span class="meta-label">Reference / Policy #</span>
                <span class="meta-value">{{ $entry->reference ?: '—' }}</span>
            </td>
            <td style="width:25%">
                <span class="meta-label">Narration</span>
                <span class="meta-value">{{ $entry->description }}</span>
            </td>
        </tr>
    </table>
    ...
    <table class="lines-table">
        ...
        <tbody>
            ...
            <tr>
                <td class="text-center" style="color:#888">{{ $loop->iteration }}</td>
                <td><span class="acc-code">{{ $line->account->account_code }}</span></td>
            ...
            <tr class="narration-row">
                <td></td>
                <td colspan="5">Being: {{ $entry->description }}</td>
                <td></td><td></td>
            </tr>
        </tbody>
        <tfoot>
            <tr>
                ...
                <td class="text-right dr-amt">{{ number_format($entry->lines->sum('debit'), 2) }}</td>
                <td class="text-right cr-amt">{{ number_format($entry->lines->sum('credit'), 2) }}</td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="sig-row">
        <div class="sig-box">
            ...
            <div style="font-size:11px;color:#555;margin-top:3px">{{ $entry->creator?->name ?? '—' }}</div>
        </div>
        <div class="sig-box">
            <div style="height:36px"></div>
