    ...
    <div style="max-width:860px;margin:0 auto;">
        ...
        <div class="pl-partner-grid" id="partnerGrid">
            ...
            <div class="pl-partner-card"
                 ...
                 data-code="{{ strtolower($partner->code ?? '') }}"
                 onclick="window.location.href='{{ route('admin.accounting.partner-ledger.show', $partner->id) }}'">
                <div class="pc-name">{{ $partner->name }}</div>
                <div class="pc-code">{{ $partner->code ?? 'No Code' }}</div>
...
document.getElementById('partnerSelectForm').addEventListener('submit', function(e) {
    ...
    if (id) {
        window.location.href = '{{ route("admin.accounting.partner-ledger.show", ["partnerId" => "__ID__"]) }}'.replace('__ID__', id);
    }
});
