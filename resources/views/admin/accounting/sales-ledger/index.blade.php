...
@section('title', 'Sales Ledger — Accounts Receivable')
...
<div class="acct-page">
    ...
    <div class="sl-summary">
        <div class="sl-sum-card dr">
            ...
            <div class="sl-sum-value dr">${{ number_format($totalDr, 2) }}</div>
        </div>
        <div class="sl-sum-card cr">
            <div class="sl-sum-label">Total Credited (Paid / Returns)</div>
            <div class="sl-sum-value cr">${{ number_format($totalCr, 2) }}</div>
        </div>
        <div class="sl-sum-card bal">
            <div class="sl-sum-label">Net AR Balance</div>
            <div class="sl-sum-value {{ $totalBalance >= 0 ? 'green' : '' }}">${{ number_format(abs($totalBalance), 2) }}
                <small style="font-size:.75rem;font-weight:500;color:var(--acct-muted)"> {{ $totalBalance >= 0 ? 'Dr' : 'Cr' }}</small>
            </div>
        </div>
    </div>
    ...
    <div class="sl-card">
        <div class="sl-card-header">
            ...
            <span style="font-size:.75rem;color:var(--acct-muted)">{{ $partnersWithAR->count() }} partners</span>
        </div>
        @if($partnersWithAR->count())
            ...
            <table class="sl-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end num-cell num-dr">${{ number_format($row->total_dr, 2) }}</td>
                        <td class="text-end num-cell num-cr">${{ number_format($row->total_cr, 2) }}</td>
                        <td class="text-center">
                            @php $bal = (float)$row->balance; @endphp
                            <span class="bal-pill {{ $bal > 0 ? 'pos' : ($bal < 0 ? 'neg' : 'zero') }}">
                                {{ $bal != 0 ? ($bal > 0 ? '' : '-') . '$' . number_format(abs($bal),2) : '$0.00' }}
                                {{ $bal > 0 ? 'Dr' : ($bal < 0 ? 'Cr' : '') }}
                            </span>
                        </td>
                        ...
                        <td class="text-center">
                            <a href="{{ route('admin.accounting.sales-ledger.partner', $row->partner_id) }}" class="btn-view-ledger">
        ...
        <div class="sl-empty">
            ...
            <a href="{{ route('admin.accounting.record-sale') }}" class="btn btn-sm mt-2" style="background:var(--acct-gold);color:#1a1a2e;font-weight:700">
