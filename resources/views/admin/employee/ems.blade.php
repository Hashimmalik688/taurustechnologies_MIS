...
<style>
    ...
    /* Strip photo toggle: greyed out = hidden from chill party strip */
    .emp-strip-toggle.a-secondary { opacity: .48; }
    .emp-strip-toggle { transition: opacity .15s, background .15s; }
...
function toggleStripPhoto(id, btn) {
    ...
    .then(function(data) {
        var showing = data.show_strip_photo;
        var icon = btn.querySelector('i');
        // Update button appearance
        btn.classList.toggle('a-info', showing);
        btn.classList.toggle('a-secondary', !showing);
        btn.style.opacity = showing ? '' : '0.55';
        if (icon) icon.className = showing ? 'bx bx-show' : 'bx bx-hide';
        btn.title = showing
            ? 'Photo showing in Chill Party strip \u2014 click to hide'
            : 'Photo hidden from Chill Party strip \u2014 click to show';
    })
    .catch(function(){ alert('Failed to update strip photo setting.'); });
}

function toggleEmpPhoto(id, btn) {
    var photoDiv = document.getElementById('emp-photo-' + id);
    if (!photoDiv) return;
    var visible = photoDiv.style.display !== 'none';
    photoDiv.style.display = visible ? 'none' : '';
    if (visible) {
        btn.classList.remove('a-info--active');
        btn.style.background = '';
        btn.style.borderColor = '';
        btn.style.color = '';
    } else {
        btn.style.background = 'rgba(80,165,241,.22)';
        btn.style.borderColor = 'rgba(80,165,241,.6)';
        btn.style.color = '#2b81c9';
    }
}
</script>
@endsection
