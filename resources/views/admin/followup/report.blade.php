...
<div class="container-fluid py-3 px-4">
    ...
    <div class="fr-filter-hero">
        ...
        <form method="GET" action="{{ route('followup.report') }}" class="fr-filter-form">
            <div class="fr-filter-row">
                ...
                <div class="fr-filter-group" style="flex-direction:row;align-items:flex-end;gap:8px;">
                    ...
                    @if(request('date_from') || request('date_to'))
                        <a href="{{ route('followup.report') }}" class="fr-btn fr-btn-ghost">
            ...
            <div class="fr-active-filters">
                ...
                @elseif(request('date_from'))
                    <span class="fr-chip"><i class="bx bx-calendar-alt"></i> From {{ \Carbon\Carbon::parse(request('date_from'))->format('M d, Y') }}</span>
                @elseif(request('date_to'))
                    <span class="fr-chip"><i class="bx bx-calendar-alt"></i> Until {{ \Carbon\Carbon::parse(request('date_to'))->format('M d, Y') }}</span>
                @endif
            </div>
            @endif
        </form>
    ...
    <div class="fr-stat-grid">
        <div class="fr-stat-card">
            ...
            <div>
                <div class="fr-stat-value">{{ number_format($totalUnassigned) }}</div>
        ...
        <div class="fr-stat-card">
            ...
            <div>
                <div class="fr-stat-value">{{ number_format($grandTotal) }}</div>
        ...
        <div class="fr-stat-card">
            ...
            <div>
                <div class="fr-stat-value">{{ number_format($grandPending) }}</div>
        ...
        <div class="fr-stat-card">
            ...
            <div>
                <div class="fr-stat-value">{{ number_format($grandDone) }}</div>
                <div class="fr-stat-label">Followup Done</div>
            </div>
        </div>
    </div>

    {{-- Unassigned leads banner --}}
    @if($totalUnassigned > 0)
    <div class="fr-unassigned-banner">
        <div class="fr-unassigned-icon"><i class="bx bx-error-circle"></i></div>
        <div>
            <div class="fr-unassigned-count">{{ $totalUnassigned }} {{ Str::plural('lead', $totalUnassigned) }}</div>
            <div class="fr-unassigned-desc">approved &amp; ready for followup but not yet assigned to anyone</div>
        </div>
        @canViewModule('issuance')
        <div class="ms-auto">
            <a href="{{ route('issuance.index') }}" class="fr-btn fr-btn-primary" style="font-size:.75rem;padding:7px 16px;">
    ...
    <div class="fr-card">
        <div class="fr-card-header">
            ...
            <span class="fr-count-badge">{{ count($summary) }} {{ Str::plural('agent', count($summary)) }}</span>
        </div>
        ...
        <div class="table-responsive">
            <table class="fr-table">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr>
                        <td><span class="fr-rank {{ $rankClass }}">{{ $rank }}</span></td>
                        <td><div class="fr-agent-name">{{ $data['name'] }}</div></td>
                        <td class="col-center">
                            <span class="fr-total-pill">{{ $data['total'] }}</span>
                        </td>
                        <td class="col-center">
                            <span class="fr-num-pill {{ $data['pending'] > 0 ? 'amber' : 'zero' }}">
                                {{ $data['pending'] }}
                            </span>
                        </td>
                        <td class="col-center">
                            <span class="fr-num-pill {{ $data['done'] > 0 ? 'green' : 'zero' }}">
                                {{ $data['done'] }}
                            </span>
                        </td>
                        <td>
                            <div class="fr-prog-label">{{ $donePct }}% complete</div>
                            <div class="fr-prog-track">
                                <div class="fr-prog-fill {{ $barClass }}" style="width:{{ $donePct }}%;"></div>
                            </div>
                        </td>
                    </tr>
                    @php $rank++; @endphp
        ...
        <div style="padding:14px 22px; background:#f8fafc; border-top:2px solid #e2e8f0; display:flex; align-items:center; gap:32px; flex-wrap:wrap;">
            ...
            <div style="display:flex;align-items:center;gap:6px;">
                ...
                <strong style="color:#1e293b;">{{ $grandTotal }}</strong>
            </div>
            <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:.75rem;color:#94a3b8;">Pending:</span>
                <strong style="color:#92400e;">{{ $grandPending }}</strong>
            </div>
            <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:.75rem;color:#94a3b8;">Done:</span>
                <strong style="color:#14532d;">{{ $grandDone }}</strong>
            </div>
            @if($grandTotal > 0)
            <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:.75rem;color:#94a3b8;">Overall:</span>
                <strong style="color:#1e293b;">{{ round(($grandDone / $grandTotal) * 100) }}% complete</strong>
            </div>
            @endif
        </div>
        ...
        <div class="fr-empty">
            ...
            <p>No followup assignments found{{ (request('month') || request('year')) ? ' for the selected period.' : '.' }}</p>
        </div>
        @endif
    </div>
