...
@section('content')
    ...
    <div class="sl-view-tabs">
        <a href="{{ route('leads.index') }}" class="sl-view-tab">
            <i class="bx bx-data"></i> Raven Leads
        </a>
        <a href="{{ route('leads.duplicates') }}" class="sl-view-tab active">
    ...
    <div class="sl-topbar">
        ...
        <div class="sl-topbar-right">
            <div class="sl-search-wrap">
                ...
                <input type="text" id="dupSearch" class="sl-search-input" placeholder="Search name, phone, SSN..." value="{{ request('search') }}">
            </div>
            <a href="{{ route('leads.index') }}" class="sl-btn sl-btn-outline">
    ...
    <div class="sl-card">
        <!-- Filter row -->
        <form method="GET" action="{{ route('leads.duplicates') }}" id="dupFilterForm" class="sl-filter-pills">
            @if(request('search'))
                <span class="sl-search-pill"><i class="bx bx-search" style="font-size:.75rem"></i>{{ request('search') }}</span>
            @endif
            @if(request()->hasAny(['search']))
                <a href="{{ route('leads.duplicates') }}" class="sl-pill-clear" title="Clear filters"><i class="bx bx-x"></i> Clear</a>
            @endif
            <span class="sl-result-count">{{ $duplicates->total() }} duplicate records</span>
        </form>
        ...
        <div class="sl-tbl-wrap">
            <table class="sl-tbl">
                ...
                <tbody>
                    @php $prevPhone = null; @endphp
                    @forelse($duplicates as $index => $lead)
                        @if($lead->phone_number !== $prevPhone)
                            @if($prevPhone !== null)
                                <tr class="dup-grp-sep"><td colspan="8">— Next duplicate group —</td></tr>
                            @endif
                            @php $prevPhone = $lead->phone_number; @endphp
                        @endif
                        <tr>
                            <td><strong>{{ $duplicates->firstItem() + $index }}</strong></td>
                            <td>
                                <div style="font-weight:600;font-size:.78rem">{{ $lead->cn_name ?? 'N/A' }}</div>
                                @if($lead->ssn)
                                    <div style="font-size:.68rem;color:#94a3b8">SSN: ***-**-{{ substr($lead->ssn, -4) }}</div>
                                @endif
                                <span class="dup-badge"><i class="bx bx-copy"></i> Duplicate</span>
                            </td>
                            <td class="dup-phone">{{ $lead->phone_number }}</td>
                            <td>
                                <span class="badge bg-secondary" style="font-size:.65rem;border-radius:22px">
                                    {{ ucfirst($lead->status ?? 'N/A') }}
                                </span>
                            </td>
                            <td style="font-size:.74rem">{{ $lead->carrier_name ?? '—' }}</td>
                            <td style="font-size:.74rem">{{ $lead->closer_name ?? '—' }}</td>
                            <td style="font-size:.72rem;color:#94a3b8">
                                {{ $lead->created_at ? $lead->created_at->format('M d, Y') : 'N/A' }}
                                <div style="font-size:.65rem">ID #{{ $lead->id }}</div>
                            </td>
                            <td>
                                @php $canonical = $canonicalLeads[$lead->phone_number] ?? null; @endphp
                                @if($canonical && $canonical->id !== $lead->id)
                                    <a href="{{ route('leads.show', $canonical->id) }}" class="dup-newer-link">
                                        <i class="bx bx-link-external"></i>
                                        {{ $canonical->cn_name ?? 'View Record' }}
                                        <span style="font-size:.65rem;color:#64748b;font-weight:600">
                                            ({{ $canonical->created_at ? $canonical->created_at->format('M d, Y') : '' }} · ID #{{ $canonical->id }})
                                        </span>
                                    </a>
                                @else
