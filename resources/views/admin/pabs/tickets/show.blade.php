...
<div class="container-fluid">
    ...
    <div class="tk-hero">
        <div class="tk-hero-left">
            <div class="tk-code-big">{{ $ticket->ticket_code }}</div>
            <div class="tk-subject-big">{{ $ticket->subject }}</div>
            <div class="tk-desc-preview">{{ Str::limit($ticket->description, 150) }}</div>
        </div>
        <div class="tk-hero-right">
            @php $slug = Str::slug($ticket->status) @endphp
            <span class="tk-status tk-{{ $slug }}">{{ $ticket->status }}</span>
            <span class="pr-{{ Str::lower($ticket->priority) }}" style="font-size:.82rem">{{ $ticket->priority }} Priority</span>
        </div>
    </div>

    {{-- Quick Action Bar --}}
    <div class="pipe-filter-bar" style="margin-bottom:1.2rem;justify-content:flex-start">
        <a href="{{ route('pabs.tickets.index') }}" class="act-btn a-info"><i class="bx bx-arrow-back"></i> Back</a>
        ...
        @if($ticket->created_by === auth()->id() || auth()->user()->hasRole([Roles::SUPER_ADMIN, Roles::CEO]))
            <form action="{{ route('pabs.tickets.destroy', $ticket) }}" method="POST" style="display:inline" onsubmit="return confirm('Are you sure you want to delete this ticket?');">
                @csrf
                @method('DELETE')
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="sec-card" style="margin-bottom:1.2rem">
                ...
                <div class="sec-body" style="font-size:.84rem;line-height:1.7;white-space:pre-line">{{ $ticket->description }}</div>
            </div>
            ...
            <div class="sec-card">
                ...
                <div class="sec-body">
                    ...
                    @if(!in_array($ticket->status, [Statuses::TICKET_CLOSED]) && ($ticket->assigned_to === auth()->id() || $ticket->created_by === auth()->id()))
                        <form action="{{ route('pabs.tickets.addComment', $ticket) }}" method="POST" class="cmt-form">
                            @csrf
            ...
            <div class="sec-card" style="margin-bottom:1.2rem">
                ...
                <div class="sec-body" style="padding:.3rem 1.1rem">
                    <div class="detail-row">
                        ...
                        <div class="detail-val" style="color:#b8860b">{{ $ticket->ticket_code }}</div>
                    </div>
                    <div class="detail-row">
                        <div class="detail-lbl">Section</div>
                        <div class="detail-val">{{ $sections[$ticket->section_id] ?? 'N/A' }}</div>
                    </div>
                    <div class="detail-row">
                        <div class="detail-lbl">Status</div>
                        <div class="detail-val"><span class="tk-status tk-{{ Str::slug($ticket->status) }}">{{ $ticket->status }}</span></div>
                    </div>
                    <div class="detail-row">
                        <div class="detail-lbl">Priority</div>
                        <div class="detail-val"><span class="pr-{{ Str::lower($ticket->priority) }}">{{ $ticket->priority }}</span></div>
                    </div>
                    <div class="detail-row">
                    ...
                    <div class="detail-row">
                        ...
                        <div class="detail-val">
                            @if($ticket->quote_amount)
                                PKR {{ number_format($ticket->quote_amount, 2) }}
                            @else
                    ...
                    <div class="detail-row">
                        ...
                        <div class="detail-val">
                            @if($ticket->total_cost)
                                PKR {{ number_format($ticket->total_cost, 2) }}
                            @else
                                <span style="color:#9ca3af">N/A</span>
                            @endif
                        </div>
                    ...
                    <div class="detail-row">
                        ...
                        <div class="detail-val">{{ $ticket->creator->name }}</div>
                    </div>
                </div>
            </div>
                ...
                <div class="sec-body" style="padding:.3rem 1.1rem">
                    @if($ticket->assignee)
                        <div class="detail-row">
                            ...
                            <div class="detail-val">{{ $ticket->assignee->name }}</div>
                        </div>
                    @else
            ...
            <form action="{{ route('pabs.tickets.update', $ticket) }}" method="POST">
                ...
                <div class="modal-body">
                    <div style="margin-bottom:1rem">
                        ...
                        <input type="text" name="subject" class="crm-input" value="{{ $ticket->subject }}" required>
                    </div>
                    <div style="margin-bottom:1rem">
                        <label class="crm-label required">Description</label>
                        <textarea name="description" class="crm-input" rows="4" required>{{ $ticket->description }}</textarea>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div style="margin-bottom:1rem">
                        ...
                        <div class="col-md-6">
                            <div style="margin-bottom:1rem">
                                ...
                                <div class="crm-prefix">
                                    ...
                                    <input type="number" name="total_cost" class="crm-input" placeholder="0.00" step="0.01" min="0" value="{{ $ticket->total_cost ?? '' }}">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div style="margin-bottom:1rem">
                                ...
                                <div class="crm-prefix">
                                    <span class="pfx">PKR</span>
                                    <input type="number" name="quote_amount" class="crm-input" placeholder="0.00" step="0.01" min="0" value="{{ $ticket->quote_amount ?? '' }}">
                                </div>
                            </div>
                        </div>
                    </div>
