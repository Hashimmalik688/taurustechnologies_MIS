...
<title>Pending Submission Report — {{ $statusLabel }}</title>
<style>
...
<div class="rpt-header">
    ...
    <div class="rpt-title">Pending Submission Report &mdash; {{ $statusLabel }}</div>
    <div class="rpt-meta">
        <span><b>Period:</b>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ ucfirst($lead->submission_status ?? 'pending') }}</td>
            <td>{{ $lead->submissionReviewer->name ?? '—' }}</td>
            <td>{{ $lead->submission_at ? \Carbon\Carbon::parse($lead->submission_at)->format('M j, Y g:i A') : '—' }}</td>
        </tr>
        @empty
        <tr>
