...
<div class="container-fluid" style="max-width:1600px">
    ...
    <div class="sec-card">
        ...
        <form method="GET" action="{{ route('submissions.index') }}" class="filter-form" id="submissionsFilterForm">
            ...
            <div>
                <label>Carrier</label>
                <select name="carrier" class="form-select" style="width:130px;" onchange="this.form.submit()">
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-1">
                                    ...
                                    <button class="a-btn a-covr btn-edit-coverage"
                                        ...
                                        data-initialdraft="{{ $lead->initial_draft_date ? \Carbon\Carbon::parse($lead->initial_draft_date)->format('Y-m-d') : '' }}"
                                        data-futuredraft="{{ $lead->future_draft_date ? \Carbon\Carbon::parse($lead->future_draft_date)->format('Y-m-d') : '' }}"
...
function setThisWeekFilter() {
    ...
    document.getElementById('filter_date_from').value = sunday.toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
    document.getElementById('filter_date_to').value = saturday.toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
    document.getElementById('submissionsFilterForm').submit();
}
...
function setThisMonthFilter() {
    ...
    document.getElementById('filter_date_from').value = firstDay.toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
    document.getElementById('filter_date_to').value = lastDay.toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
    document.getElementById('submissionsFilterForm').submit();
}
</script>
@endsection
