...
@section('title')
    Submission Drilldown — {{ $carrierLabel }}{{ $partnerLabel ? ' · '.$partnerLabel : '' }}
@endsection
...
<div class="dd-hdr">
    <div class="dd-hdr-left">
        ...
        <div class="dd-breadcrumb">
            ...
            <a href="{{ $backUrl }}">Submission Performance</a>
            <span class="dd-bc-sep">/</span>
            <span class="dd-bc-current">
                {{ $carrierLabel }}{{ $partnerLabel ? ' · '.$partnerLabel : '' }}
            </span>
        </div>
    </div>
</div>
...
<div class="dd-ctx">
    ...
    @if($partnerLabel)
        <span class="dd-partner-badge">{{ $partnerLabel }}</span>
    @endif
...
<div class="dd-kpis">
    ...
    <div class="dd-kpi dd-k-covered">
        ...
        <div class="dd-kpi-val">{{ $hasRevCount }}</div>
    ...
    <div class="dd-kpi dd-k-missing">
        ...
        <div class="dd-kpi-val">{{ $noRevCount }}</div>
        <div class="dd-kpi-sub">{{ $noRevPct }}% — no rate configured</div>
    </div>
</div>

{{-- ── Why no revenue? Alert ─────────────────────────── --}}
@if($noRevCount > 0)
<div class="dd-alert">
    <i class="bx bx-info-circle"></i>
    <div class="dd-alert-body">
        <strong>{{ $noRevCount }} sale{{ $noRevCount !== 1 ? 's' : '' }} show "— no rate"</strong>
        ...
        To fix: add settlement % rates for <strong>{{ $carrierLabel }}{{ $partnerLabel ? ' · '.$partnerLabel : '' }}</strong>
...
<div class="dd-card">
    <div class="dd-card-head">
        <h6>
            ...
            Individual Sales — {{ $carrierLabel }}{{ $partnerLabel ? ' · '.$partnerLabel : '' }}
        </h6>
        ...
        <table class="dd-tbl">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="td-r">
                        @if($hasRevenue)
                            <span class="dd-chip dd-chip-indigo" title="Source: {{ $revSource }}{{ $revChipExtra ? ' (estimated from configured rate)' : '' }}">
                                <i class="bx bx-trending-up" style="font-size:.68rem"></i>{{ number_format($lead->eff_revenue, 2) }}{{ $revChipExtra }}
                            </span>
                        @elseif($revSource === 'no_premium')
                    ...
                    <td class="dd-pct">
                        @if($commPct)
                            <span class="dd-pct-val">{{ number_format($commPct, 1) }}%</span>
                        @else
            ...
            <tfoot>
                <tr>
                    ...
                    <td colspan="4" style="font-size:.62rem;color:var(--dd-text-4)">
                        avg ${{ number_format($avgRevPerSale, 2) }} / sale
                        @if($noRevCount > 0)
                        &nbsp;·&nbsp; <span style="color:var(--dd-rose)">{{ $noRevCount }} missing commission</span>
                        @endif
                    </td>
                </tr>
            </tfoot>
