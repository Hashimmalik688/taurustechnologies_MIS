...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div class="stat-card-value" id="sc-total">{{ number_format($summaryTotalCalls) }}</div>
    </div>
    <div class="stat-card">
        <i class="bx bx-time stat-card-icon"></i>
        <div class="stat-card-label">Total Talk Time</div>
        <div class="stat-card-value" id="sc-duration" style="font-size:1.3rem">{{ $totalDurFmt }}</div>
    </div>
    <div class="stat-card">
    ...
    <div class="stat-card">
        ...
        <div class="stat-card-value" id="sc-rate">{{ $connectRate }}%</div>
    </div>
    <div class="stat-card">
        <i class="bx bx-group stat-card-icon"></i>
        <div class="stat-card-label">Agents Tracked</div>
        <div class="stat-card-value" id="sc-agents">{{ $agentKpis->count() }}</div>
    </div>
</div>
    ...
    <div style="overflow-x:auto">
        <table class="ap-table" id="apTable">
            ...
            <tbody id="apBody">
                ...
                @endphp
                <tr
                    data-key="{{ $agent['extension'] ?: $agent['name'] }}"
            ...
            <tfoot id="apFoot">
                <tr>
                    <td>
                        TOTAL
                        <span style="font-size:.6rem;font-weight:400;color:var(--bs-surface-500)" id="agentCount">({{ $agentKpis->count() }} agents)</span>
                    </td>
...
const DATA_URL    = '{{ route('settings.reports.zoom-agent-performance.data') }}' +
