...
@section('content')
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-card-value">{{ number_format($stats['calls_with_recording']) }}</div>
        </div>
        <div class="stat-card">
    ...
    <div style="margin-bottom:1rem">
        <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem">
            ...
            <h6 style="margin:0;font-size:.85rem;font-weight:700;color:var(--bs-surface-700)">
                {{ request('user_filter') }} - Performance Breakdown
            </h6>
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-card-value" style="font-size:1.3rem">{{ gmdate('H:i:s', $userStats['total_talk_time']) }}</div>
                <div style="font-size:.6rem;color:var(--bs-surface-500);margin-top:.15rem">
            ...
            <form method="GET" action="{{ route('settings.reports.zoom-logs') }}" id="filterForm">
                <div style="display:flex;gap:.55rem;align-items:flex-end;flex-wrap:wrap">
                    ...
                    <div>
                        ...
                        <input type="text" name="user_filter" value="{{ request('user_filter') }}" 
                    ...
                    <div>
                        ...
                        <select name="call_status" style="font-size:.72rem;padding:.3rem .5rem;border:1px solid rgba(0,0,0,.1);border-radius:8px;background:#fff;min-width:140px">
                            ...
                            @foreach($callStatuses as $key => $label)
                                <option value="{{ $key }}" {{ request('call_status') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
    ...
    <div class="ex-card sec-card">
                            ...
                            <tr>
                                <td style="white-space:nowrap">
                                    @if($log->call_start_time)
                                        <div style="font-weight:600">{{ $log->call_start_time->format('m/d/Y') }}</div>
                                        <div style="font-size:.65rem;color:var(--bs-surface-500)">{{ $log->call_start_time->format('g:i A') }}</div>
                                    @else
                                ...
                                <td>
                                    @if($log->caller_name)
                                        <div style="font-weight:600">{{ $log->caller_name }}</div>
                                    @endif
                                    @if($log->caller_number)
                                        <div style="font-family:monospace;font-size:.7rem;color:var(--bs-surface-600)">{{ $log->caller_number }}</div>
                                    @endif
                                    @if($log->caller_email)
                                        <div style="font-size:.65rem;color:var(--bs-surface-500)">{{ $log->caller_email }}</div>
                                    @endif
                                    @if(!$log->caller_name && !$log->caller_number && !$log->caller_email)
                                        <span style="color:var(--bs-surface-400)">—</span>
                                    @endif
                                </td>
                                <td>
                                    @if($log->callee_name)
                                        <div style="font-weight:600">{{ $log->callee_name }}</div>
                                    @endif
                                    @if($log->callee_number)
                                        <div style="font-family:monospace;font-size:.7rem;color:var(--bs-surface-600)">{{ $log->callee_number }}</div>
                                    @endif
                                    @if(!$log->callee_name && !$log->callee_number)
                                        <span style="color:var(--bs-surface-400)">—</span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    <div style="display:flex;gap:.25rem;flex-wrap:wrap">
                                        @if($log->zoom_call_id)
                                            <button type="button" 
                                                    ...
                                                    onclick="navigator.clipboard.writeText('{{ $log->zoom_call_id }}'); this.innerHTML='<i class=\"bx bx-check\"></i>'; setTimeout(() => this.innerHTML='<i class=\"bx bx-copy\"></i>', 1000)">
                                        ...
                                        @if($log->lead_id && $log->lead)
                                            <a href="{{ route('leads.show', $log->lead_id) }}" 
                                               ...
                                               style="font-size:.65rem;padding:.2rem .45rem" 
                                               title="View in MIS: {{ $log->lead->cn_name }}">
        ...
        @else
            <div class="rp-empty">
                ...
                <p>
                    @if(request()->hasAny(['search', 'agent_id', 'call_status', 'call_type', 'event_type', 'has_recording', 'date_from', 'date_to']))
