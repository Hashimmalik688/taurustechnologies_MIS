    ...
    <div class="sl-kpi-row">
        <div class="sl-kpi-pill">
            ...
            <div>
                ...
                <div class="kpi-value text-warning">{{ $not_issued_count }}</div>
            </div>
        </div>
        <div class="sl-kpi-pill">
            ...
            <div>
                ...
                <div class="kpi-value text-danger">{{ $not_paid_count }}</div>
            </div>
        </div>
    </div>
                        ...
                        <tbody>
                            @forelse($not_issued_leads as $lead)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('leads.show', $lead->id) }}" style="font-weight:600;font-size:.73rem;color:var(--bs-body-color);text-decoration:none;" target="_blank">
                                            {{ $lead->cn_name ?? '—' }}
                                        </a>
                                    </td>
                                    ...
                                    <td style="font-size:.7rem;">{{ $lead->sale_date ? \Carbon\Carbon::parse($lead->sale_date)->format('M d, Y') : '—' }}</td>
                                    <td>
                                        <span class="badge bg-warning text-dark" style="font-size:.65rem;">
                                            {{ \App\Support\Statuses::NOT_ISSUED_DISPOSITIONS[$lead->not_issued_disposition] ?? $lead->not_issued_disposition ?? '—' }}
                                        </span>
                                    </td>
                                    <td style="font-size:.71rem;">{{ $lead->notIssuedBy->name ?? '—' }}</td>
                                    <td style="font-size:.7rem;">{{ $lead->not_issued_at ? $lead->not_issued_at->format('M d, Y') : '—' }}</td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-danger" style="font-size:.65rem;">
                                            {{ \App\Support\Statuses::FDFP_TYPES[$lead->not_paid_fdfp_type] ?? $lead->not_paid_fdfp_type ?? '—' }}
                                        </span>
                                        @if($lead->not_paid_fdfp_type === 'manual_action' && $lead->not_paid_manual_disposition)
                                            <div style="font-size:.62rem;color:#94a3b8;margin-top:1px;">
                                                → {{ \App\Support\Statuses::NOT_ISSUED_DISPOSITIONS[$lead->not_paid_manual_disposition] ?? $lead->not_paid_manual_disposition }}
                                            </div>
                                        @endif
                                    </td>
