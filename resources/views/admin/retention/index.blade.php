...
<div class="ret-kpi-row">
    <a href="{{ $kpiLinks['pending'] }}" class="ret-kpi-pill k-pending {{ ($disp_filter ?? '') === 'pending' ? 'kpi-active' : '' }}">
        <div class="rk-icon"><i class="bx bx-time-five"></i></div>
        <div><div class="rk-lbl">Pending</div><div class="rk-val">{{ $kpi['pending'] ?? 0 }}</div></div>
    </a>
    <a href="{{ $kpiLinks['retained'] }}" class="ret-kpi-pill k-retained {{ ($disp_filter ?? '') === 'retained' ? 'kpi-active' : '' }}">
    ...
    <a href="{{ $kpiLinks['in_progress'] }}" class="ret-kpi-pill k-in_progress {{ ($disp_filter ?? '') === 'in_progress' ? 'kpi-active' : '' }}">
    ...
    <a href="{{ $kpiLinks['rewrite'] }}" class="ret-kpi-pill k-rewrite {{ ($disp_filter ?? '') === 'rewrite' ? 'kpi-active' : '' }}">
        <div class="rk-icon"><i class="bx bx-edit-alt"></i></div>
        <div><div class="rk-lbl">Rewrite</div><div class="rk-val">{{ $kpi['rewrite'] ?? 0 }}</div></div>
    </a>
    <a href="{{ $kpiLinks['recalled_to_closer'] }}" class="ret-kpi-pill k-recalled {{ ($disp_filter ?? '') === 'recalled_to_closer' ? 'kpi-active' : '' }}">
...
<div class="sl-card">
    ...
    <form method="GET" action="{{ route('retention.index') }}" id="retFilterForm" class="sl-filter-pills">
        ...
        @if($disp_filter ?? null)<input type="hidden" name="disp_filter" value="{{ $disp_filter }}">@endif
        ...
        @if(request()->hasAny(['search','month','year','date_from','date_to','disp_filter']))
    ...
    <div class="sl-tabs" role="tablist">
        ...
        {{-- Toggle disposed --}}
        <a href="{{ route('retention.index', array_merge(request()->except(['disposed', 'disp_filter']), $disposed && !($disp_filter ?? null) ? [] : ['disposed' => 1])) }}"
        ...
        <div class="tab-pane" id="tab-cancelled" role="tabpanel">
            <div class="sl-tbl-wrap">
                <table class="sl-tbl">
                    ...
                    <tbody>
                        @forelse($cancelled_leads as $lead)
                        @php
                            $retDisp = $lead->retention_disposition ?: 'pending';
                            $beneficiaries = $lead->beneficiaries ?? [];
                            if(is_string($beneficiaries)){$d=json_decode($beneficiaries,true);$beneficiaries=is_array($d)?$d:[];}
            ...
            @if($cancelled_leads->hasPages())
                <div class="px-3 py-2">{{ $cancelled_leads->withQueryString()->links() }}</div>
            @endif
        </div>
