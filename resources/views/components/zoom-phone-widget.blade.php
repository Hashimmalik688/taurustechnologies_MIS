{{-- ┌─────────────────────────────────────────────────────────────────────────┐
     │  Floating Zoom Phone Widget — appears on every CRM page               │
     │  Bottom-right FAB → opens a mini dialer panel                         │
     └─────────────────────────────────────────────────────────────────────────┘ --}}
...
<div class="zpw-panel" id="zpwPanel">
    <div class="zpw-hdr">
        ...
        <div class="zpw-hdr-actions">
            <a href="{{ route('zoom.phone') }}" title="Full screen"><i class="bx bx-expand"></i></a>
    ...
    <div class="zpw-footer">
        <a href="{{ route('zoom.phone') }}"><i class="bx bx-expand"></i> Open Full Dialer</a>
    </div>
</div>
...
<script>
    ...
    async function loadRecentCalls() {
        ...
        try {
            const resp = await fetch('{{ route("settings.reports.zoom-logs") }}?_widget=1&per_page=8', {
            ...
            listDiv.innerHTML = `
                <div style="text-align:center;padding:.75rem;font-size:.68rem;color:var(--bs-surface-500)">
                    ...
                    <a href="{{ route('settings.reports.zoom-logs') }}" style="color:var(--bs-gold,#d4af37);font-weight:600">View all call logs →</a>
                </div>
            `;
            recentLoaded = true;
        } catch (e) {
