    ...
    <div class="row g-3 mt-1">
        ...
        <div class="col-xl-5 col-lg-6">
            ...
            <div class="dk-card dk-banner">
                <div class="dk-user">
                    <div class="dk-avatar">{{ substr($user->name, 0, 1) }}</div>
                    <div>
                        <div class="dk-name">{{ $user->name }}</div>
                        <span class="dk-role">
                ...
                <div class="dk-total-val">
                    <div class="amt">Rs {{ number_format($totalDocked, 0) }}</div>
            ...
            <div class="dk-stats">
                <div class="dk-stat s-active dk-card">
                    <div class="s-val" style="font-size:1rem">Rs {{ number_format($totalDocked, 0) }}</div>
                    <div class="s-lbl">Active Amount</div>
                </div>
                <div class="dk-stat s-applied dk-card">
                    <div class="s-val" style="font-size:1rem">Rs {{ number_format($totalApplied, 0) }}</div>
                    <div class="s-lbl">Applied Amount</div>
                </div>
                <div class="dk-stat s-amount dk-card">
                    <div class="s-val" style="font-size:1rem">Rs {{ number_format($totalDocked + $totalApplied, 0) }}</div>
        ...
        <div class="col-xl-7 col-lg-6">
            ...
            <div class="dk-card dk-table-wrap">
                <div class="tbl-hdr">
                    ...
                    <span class="badge-count">{{ $dockRecords->total() }} records</span>
                </div>
                ...
                <div class="table-responsive">
                    <table class="dk-tbl">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <span style="font-weight:600">{{ $record->dock_date->format('d') }}</span>
                                    <span style="color:var(--bs-surface-500)">{{ $record->dock_date->format('M Y') }}</span>
                                </td>
                                <td>
                                    <span class="dk-amt">
                                        <i class="bx bx-minus"></i>
                                        Rs {{ number_format($record->amount, 0) }}
                                    </span>
                                </td>
                                <td>
                                    <div class="dk-reason">
                                        <div class="main">{{ $record->reason }}</div>
                                        @if ($record->notes)
                                            <div class="note">
                                                <i class="bx bx-note"></i> {{ Str::limit($record->notes, 50) }}
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    <div class="dk-by">
                                        <div class="by-avatar">{{ substr($record->dockedBy->name ?? '?', 0, 1) }}</div>
                                        <div>
                                            <div class="by-name">{{ $record->dockedBy->name ?? 'Unknown' }}</div>
                                            <div class="by-time">{{ $record->created_at->format('g:i A') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td style="white-space:nowrap;font-size:.72rem">
                                    {{ \Carbon\Carbon::create($record->dock_year, $record->dock_month)->format('M Y') }}
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    (function(){
        ...
        @foreach($monthlyData as $m)
            labels.push('{{ \Carbon\Carbon::create($m->dock_year, $m->dock_month)->format("M Y") }}');
            amounts.push({{ $m->total }});
            counts.push({{ $m->count }});
        @endforeach
    ...
    (function(){
        ...
        @foreach($reasonBreakdown as $rb)
            reasons.push({!! json_encode(Str::limit($rb->reason, 20)) !!});
            amounts.push({{ $rb->total }});
        @endforeach
