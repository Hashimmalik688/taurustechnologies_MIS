    ...
    <div class="np-main">
        ...
        <div class="np-editor-pane">
            <div class="np-editor-titlebar" id="npTitleBar" style="display:none">
                ...
                {{-- Owner badge (shared-by-others notes) --}}
...
<div class="np-overlay" id="shareModal">
    <div class="np-modal">
        ...
        <div class="np-user-list" id="shareUserList">
            @foreach($shareableUsers as $su)
            <div class="np-user-row" data-uid="{{ $su->id }}" data-name="{{ strtolower($su->name) }}" onclick="toggleShareUser(this)">
                <input type="checkbox" id="su{{ $su->id }}" value="{{ $su->id }}" onclick="event.stopPropagation()">
                <label for="su{{ $su->id }}">{{ $su->name }}</label>
            </div>
            @endforeach
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    notes.push({
        id:             {{ $n->id }},
        title:          {!! json_encode($n->title ?? 'Untitled') !!},
        content:        {!! json_encode($n->content ?? '') !!},
        is_shared:      {{ $n->is_shared ? 'true' : 'false' }},
        shared_with:    [{{ $n->sharedWith->pluck('id')->join(',') }}],
        is_mine:        true,
        updated_at:     {!! json_encode($n->updated_at->toIso8601String()) !!},
        updated_ago:    {!! json_encode($n->updated_at->diffForHumans()) !!},
    });
    @endforeach

    @foreach($sharedNotes as $n)
    notes.push({
        id:             {{ $n->id }},
        title:          {!! json_encode($n->title ?? 'Untitled') !!},
        content:        {!! json_encode($n->content ?? '') !!},
        ...
        is_mine:        false,
        can_edit:       true,
        owner:          {!! json_encode($n->user?->name ?? 'Someone') !!},
        updated_at:     {!! json_encode($n->updated_at->toIso8601String()) !!},
        updated_ago:    {!! json_encode($n->updated_at->diffForHumans()) !!},
    });
    @endforeach
