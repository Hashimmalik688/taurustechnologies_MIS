...
@section('title') Carriers & States @endsection
    ...
    <div class="col-lg-8">
        <div class="pd-card">
            <div class="pd-head">
                ...
                <span class="pd-count">{{ $carrierStates->count() }}</span>
            </div>
            <div class="pd-body">
                ...
                <div class="pc-carrier">
                    <div class="pc-carrier-hdr">
                        <div class="pc-carrier-name">
                            ...
                            {{ $cs['carrier']->name ?? 'Unknown Carrier' }}
                        </div>
                        <div class="pc-rates">
                            @if($cs['settlement_level_pct'])
                            <span class="pc-rate pc-rate-level">
                                <span class="pc-rate-lbl">Level</span> {{ $cs['settlement_level_pct'] }}%
                            </span>
                            @endif
                            @if($cs['settlement_graded_pct'])
                            <span class="pc-rate pc-rate-graded">
                                <span class="pc-rate-lbl">Graded</span> {{ $cs['settlement_graded_pct'] }}%
                            </span>
                            @endif
                            @if($cs['settlement_gi_pct'])
                            <span class="pc-rate pc-rate-gi">
                                <span class="pc-rate-lbl">GI</span> {{ $cs['settlement_gi_pct'] }}%
                            </span>
                            @endif
                            @if($cs['settlement_modified_pct'])
                            <span class="pc-rate pc-rate-modified">
                                <span class="pc-rate-lbl">Modified</span> {{ $cs['settlement_modified_pct'] }}%
                            </span>
                            @endif
                        </div>
                    </div>
                    <div style="font-size:.72rem;color:#9ca3af;font-weight:600;margin-bottom:.35rem;">
                        ...
                        {{ $cs['state_count'] }} {{ $cs['state_count'] == 1 ? 'state' : 'states' }} authorized
                    </div>
                    <div class="pc-states">
                        @foreach($cs['states'] as $st)
    ...
    <div class="col-lg-4">
            ...
            <div class="pd-body" style="padding:.85rem 1.1rem;">
                <div style="display:flex;flex-direction:column;gap:.6rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:.5rem .65rem;background:rgba(79,70,229,.05);border-radius:.35rem;border:1px solid rgba(79,70,229,.1);">
                        ...
                        <span style="font-size:1.1rem;font-weight:900;color:#4f46e5;">{{ $activeCarriers->count() }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:.5rem .65rem;background:rgba(5,150,105,.05);border-radius:.35rem;border:1px solid rgba(5,150,105,.1);">
                        ...
                        <span style="font-size:1.1rem;font-weight:900;color:#059669;">{{ $authorizedStates->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="pc-all-states-card">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:.25rem;">
                @foreach($authorizedStates as $st)
                <span class="pd-state-pill">{{ $st }}</span>
                @endforeach
            </div>
        </div>
