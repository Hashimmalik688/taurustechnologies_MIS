...
@section('content')
    ...
    {{-- QA Score Card --}}
    @if(isset($qaStats) && $qaStats['total_calls'] > 0)
    <div class="ex-card sec-card" style="margin-bottom:1rem;">
        <div class="pipe-hdr" style="color:#6f42c1;">
            ...
            <span style="font-size:.7rem;font-weight:500;color:var(--bs-surface-400);margin-left:.4rem;">Last 30 Days · {{ $qaStats['total_calls'] }} scored call{{ $qaStats['total_calls'] !== 1 ? 's' : '' }}</span>
            <a href="{{ route('qa.my-report') }}" style="margin-left:auto;font-size:.72rem;font-weight:600;color:#6f42c1;text-decoration:none;display:inline-flex;align-items:center;gap:.25rem;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:.75rem;padding:.5rem .65rem 0;">
            ...
            @endphp
            <div style="flex:1;min-width:110px;background:{{ $scoreBg }};border-radius:12px;padding:.6rem .85rem;border:1px solid {{ $scoreColor }}22;text-align:center;">
                <div style="font-size:1.4rem;font-weight:800;color:{{ $scoreColor }};line-height:1.1;">{{ $qaStats['avg_score'] }}</div>
            ...
            @endphp
            <div style="flex:1;min-width:110px;background:{{ $compBg }};border-radius:12px;padding:.6rem .85rem;border:1px solid {{ $compColor }}22;text-align:center;">
                <div style="font-size:1.4rem;font-weight:800;color:{{ $compColor }};line-height:1.1;">{{ $qaStats['compliance_rate'] }}%</div>
            ...
            <div style="flex:1;min-width:100px;background:rgba(16,185,129,.06);border-radius:12px;padding:.6rem .85rem;border:1px solid rgba(16,185,129,.12);text-align:center;">
                <div style="font-size:1.4rem;font-weight:800;color:#16a34a;line-height:1.1;">{{ $qaStats['good_calls'] }}</div>
            ...
            <div style="flex:1;min-width:100px;background:rgba(239,68,68,.06);border-radius:12px;padding:.6rem .85rem;border:1px solid rgba(239,68,68,.10);text-align:center;">
                <div style="font-size:1.4rem;font-weight:800;color:#dc2626;line-height:1.1;">{{ $qaStats['void_risks'] }}</div>
            ...
            <div style="flex:1;min-width:110px;background:rgba(239,68,68,.06);border-radius:12px;padding:.6rem .85rem;border:1px solid rgba(239,68,68,.10);text-align:center;">
                <div style="font-size:1.4rem;font-weight:800;color:#dc2626;line-height:1.1;">{{ $qaStats['compliance_fails'] }}</div>
        ...
        {{-- Recent scored calls --}}
        @if(count($qaStats['recent_calls']) > 0)
        <div style="padding:.6rem .65rem .65rem;">
            ...
            <div class="scroll-tbl" style="max-height:220px;">
                <table class="ex-tbl">
                    ...
                    <tbody>
                        @foreach($qaStats['recent_calls'] as $qc)
                        @php
                            $qcScore = $qc['score'];
                        ...
                        @endphp
                        <tr>
                            <td style="white-space:nowrap;font-size:.72rem;">{{ $qc['call_date'] }}</td>
                            <td style="font-size:.75rem;max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $qc['customer'] }}</td>
                            <td class="text-center">
                                <span style="font-weight:800;font-size:.82rem;color:{{ $qcColor }};">{{ number_format($qc['score'], 1) }}</span>
                            </td>
                            <td class="text-center">
                                <span class="st-pill" style="background:{{ $ds['bg'] }};color:{{ $ds['color'] }};border:1px solid {{ $ds['color'] }}22;">
                                    {{ str_replace('_',' ', $qc['disposition']) }}
                                </span>
                            </td>
                            <td class="text-center">
                                @if($qc['compliance'])
                            ...
                            <td style="font-size:.7rem;color:var(--bs-surface-500);max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="{{ $qc['top_issue'] ?? '' }}">
                                {{ $qc['top_issue'] ?? '—' }}
                            </td>
                        </tr>
                        @endforeach
