...
<div class="cs-page-hdr">
    <h5>
        <i class="bx bx-ghost"></i> Ghost Mode
...
<div class="cs-tabs">
    <button class="cs-tab-btn active" data-tab="chat" onclick="switchTab('chat')">
        <i class="bx bx-message-square-dots"></i> Chat Shadows
    </button>
    <button class="cs-tab-btn" data-tab="notes" onclick="switchTab('notes')">
        <i class="bx bx-note"></i> Sticky Shadows
        <span class="tab-count" id="notesTabCount">0</span>
    </button>
    <button class="cs-tab-btn" data-tab="notepad" onclick="switchTab('notepad')">
        <i class="bx bx-notepad"></i> Notepad Shadows
        <span class="tab-count" id="notepadTabCount">0</span>
    </button>
</div>
...
<!-- ═══ TAB 3: Notepad Shadowing ═══ -->
<div class="cs-tab-pane" id="tabNotepad">
    <!-- Notepad KPIs -->
    <div class="kpi-row" id="npKpiRow">
        <div class="ex-card kpi-card k-gold">
            <i class="bx bx-notepad k-icon"></i>
            <div class="k-val" id="npkTotal">&mdash;</div>
            <div class="k-lbl">Total Notes</div>
        </div>
        <div class="ex-card kpi-card k-blue">
            <i class="bx bx-user k-icon"></i>
            <div class="k-val" id="npkUsers">&mdash;</div>
            <div class="k-lbl">Users</div>
        </div>
        <div class="ex-card kpi-card k-green">
            <i class="bx bx-pin k-icon"></i>
            <div class="k-val" id="npkPinned">&mdash;</div>
            <div class="k-lbl">Pinned</div>
        </div>
        <div class="ex-card kpi-card k-red">
            <i class="bx bx-trash k-icon"></i>
            <div class="k-val" id="npkDeleted">&mdash;</div>
            <div class="k-lbl">Deleted</div>
        </div>
        <div class="ex-card kpi-card k-purple">
            <i class="bx bx-share-alt k-icon"></i>
            <div class="k-val" id="npkShared">&mdash;</div>
            <div class="k-lbl">Shared</div>
        </div>
    </div>

    <!-- Notepad Filters -->
    <div class="ns-filters">
        <input type="text" class="form-control" id="npSearch" placeholder="Search notepad notes..." autocomplete="off">
        <select class="form-select" id="npUserFilter">
            <option value="all">All Users</option>
        </select>
        <select class="form-select" id="npStatusFilter">
            <option value="all">All Notes</option>
            <option value="active">Active Only</option>
            <option value="deleted">Deleted Only</option>
    ...
    <div id="notepadContainer">
        <div class="cs-spin" id="notepadSpin"><div class="spinner-border text-secondary"></div></div>
    </div>

    <!-- Notepad Preview Modal -->
    <div id="npPreviewModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:3000;align-items:center;justify-content:center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.switchTab = function(tab) {
        ...
        const paneMap = { chat: 'tabChat', notes: 'tabNotes', notepad: 'tabNotepad' };
        document.getElementById(paneMap[tab] || 'tabChat').classList.add('active');
        if (tab === 'notes' && !notesLoaded) {
            notesLoaded = true;
            loadNotes();
        }
        if (tab === 'notepad' && !notepadLoaded) {
            notepadLoaded = true;
            loadNotepadNotes();
        }
    };
    ...
    // NOTEPAD SHADOWING
    // ══════════════════════════════════════════════
    const notepadContainer = $('notepadContainer');
    const notepadSpin      = $('notepadSpin');
    const npSearch         = $('npSearch');
    const npUserFilter     = $('npUserFilter');
    const npkTotal         = $('npkTotal');
    const npkUsers         = $('npkUsers');
    const npkDeleted        = $('npkDeleted');
    const npkShared         = $('npkShared');
    const npStatusFilter    = $('npStatusFilter');
    const npkPinned         = $('npkPinned');
    const notepadTabCount   = $('notepadTabCount');
    let npTimer = null;

    function loadNotepadNotes() {
        notepadSpin.style.display = 'flex';
        const p = new URLSearchParams({
            search: npSearch.value,
        ...
        .then(d => {
            ...
            if (!d.success) return;
            notepadTabCount.textContent = d.total;
            npkTotal.textContent  = d.total;
            npkUsers.textContent  = d.users.length;
            if (npkDeleted) npkDeleted.textContent = d.deleted_count || 0;
            if (npkShared)  npkShared.textContent  = d.shared_count  || 0;

            if (npUserFilter.options.length <= 1 && d.users.length > 0) {
                d.users.forEach(u => {
                    const opt = document.createElement('option');
                    opt.value = u.id;
                    opt.textContent = u.name;
                    npUserFilter.appendChild(opt);
                });
            }

            // Count pinned
            const pinnedCount = d.notes.filter(n => n.is_pinned).length;
            npkPinned.textContent = pinnedCount;

            renderNotepadNotes(d.notes);
        })
        .catch(() => {
            notepadSpin.style.display = 'none';
            notepadContainer.innerHTML = '<div style="text-align:center;padding:2rem;opacity:.5"><i class="bx bx-error-circle"></i> Failed to load notepad notes</div>';
        });
    }

    function openNotepadPreview(note) {
        document.getElementById('npPreviewTitle').textContent = note.title || 'Untitled';
        document.getElementById('npPreviewMeta').textContent  = (note.user_name || '') + ' · Updated ' + (note.updated_ago || '');
        document.getElementById('npPreviewBody').textContent  = note.content || '(empty)';
        const badge = document.getElementById('npSharedBadge');
        if (note.is_shared) {
            badge.style.display = 'block';
            const label = document.getElementById('npSharedWithText');
            if (label) label.textContent = note.shared_with_names
                ? 'Shared with: ' + note.shared_with_names
                : 'Shared (no active recipients)';
        } else {
            badge.style.display = 'none';
        }
        const modal = document.getElementById('npPreviewModal');
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape') {
            const m = document.getElementById('npPreviewModal');
            if (m) m.style.display = 'none';
        }
    });

    function renderNotepadNotes(notes) {
        if (!notes.length) {
            notepadContainer.innerHTML = '<div style="text-align:center;padding:3rem 1rem;opacity:.45"><i class="bx bx-notepad" style="font-size:2.5rem;display:block;margin-bottom:.5rem"></i><p style="font-size:.8rem">No notepad notes found</p></div>';
            return;
        }
        ...
        const byUser = {};
        notes.forEach(n => {
            if (!byUser[n.user_id]) byUser[n.user_id] = { name: n.user_name, notes: [] };
            byUser[n.user_id].notes.push(n);
        });

        let html = '';
        const allNotes = []; // for onclick closure
        Object.values(byUser).forEach(user => {
            html += `<div class="mb-3">
                <div class="sec-hdr" style="padding:.4rem .6rem;margin-bottom:.5rem">
                    <h6 style="font-size:.75rem"><i class="bx bx-user"></i> ${esc(user.name)}</h6>
                    <span class="bd-mini bd-blue">${user.notes.length}</span>
                </div>
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:.6rem;padding:0 .1rem">`;
            user.notes.forEach(n => {
                allNotes.push(n);
                const idx     = allNotes.length - 1;
                const bg      = n.color || '#1e2235';
                const light   = isLightColor(bg);
                const txtClr  = light ? 'rgba(0,0,0,.8)' : 'rgba(255,255,255,.9)';
                const subClr  = light ? 'rgba(0,0,0,.45)' : 'rgba(255,255,255,.5)';
                const border  = light ? 'rgba(0,0,0,.06)' : 'rgba(255,255,255,.1)';
                const pinIcon  = n.is_pinned ? '<i class="bx bxs-pin" style="font-size:.72rem;color:#c84646;margin-right:.25rem"></i>' : '';
                const shIcon   = n.is_shared ? '<i class="bx bxs-share-alt" style="font-size:.72rem;color:#34c38f;margin-right:.25rem" title="Shared"></i>' : '';
                const delIcon  = n.is_deleted ? '<i class="bx bx-trash" style="font-size:.72rem;color:#e74c3c;margin-right:.25rem" title="Deleted"></i>' : '';
                const delBorder = n.is_deleted ? '1.5px dashed #e74c3c' : `1px solid ${border}`;
                const delBadge = n.is_deleted && n.deleted_ago ? `<div style="font-size:.58rem;color:#e74c3c;margin-top:.3rem"><i class="bx bx-trash"></i> Deleted ${esc(n.deleted_ago)}</div>` : '';
                const sharedInfo = n.is_shared && n.shared_with_names ? `<div style="font-size:.58rem;color:#34c38f;margin-top:.2rem"><i class="bx bxs-user-check"></i> ${esc(n.shared_with_names)}</div>` : '';
                const preview  = (n.content || '').slice(0, 140);
                html += `<div data-np-idx="${idx}" style="background:${bg};border-radius:6px;padding:.7rem .8rem;border:${delBorder};box-shadow:0 2px 6px rgba(0,0,0,.12);cursor:pointer;transition:transform .12s,box-shadow .12s;opacity:${n.is_deleted ? '.72' : '1'}" onmouseenter="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 18px rgba(0,0,0,.2)'" onmouseleave="this.style.transform='';this.style.boxShadow='0 2px 6px rgba(0,0,0,.12)'">
                    <div style="font-size:.78rem;font-weight:700;margin-bottom:.3rem;color:${txtClr};display:flex;align-items:center">${delIcon}${pinIcon}${shIcon}${esc(n.title || 'Untitled')}</div>
                    ${preview ? `<div style="font-size:.72rem;color:${txtClr};opacity:.82;line-height:1.55;overflow:hidden;display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical;white-space:pre-wrap">${esc(preview)}</div>` : ''}
                    ${delBadge}${sharedInfo}
                    <div style="font-size:.6rem;color:${subClr};margin-top:.45rem;display:flex;justify-content:space-between">
                        <span>${esc(n.updated_ago)}</span>
                        <span style="opacity:.55">Click to open</span>
                    </div>
                </div>`;
            });
            html += '</div></div>';
        ...
        notepadContainer.innerHTML = html;

        // Wire up click handlers after DOM update
        notepadContainer.querySelectorAll('[data-np-idx]').forEach(el => {
            const idx = parseInt(el.getAttribute('data-np-idx'));
            el.addEventListener('click', () => openNotepadPreview(allNotes[idx]));
        });
    }

    if (npSearch) {
        npSearch.addEventListener('input', () => { clearTimeout(npTimer); npTimer = setTimeout(() => { if (notepadLoaded) loadNotepadNotes(); }, 350); });
    }
    if (npUserFilter) {
        npUserFilter.addEventListener('change', () => { if (notepadLoaded) loadNotepadNotes(); });
    }
    if (npStatusFilter) {
        npStatusFilter.addEventListener('change', () => { if (notepadLoaded) loadNotepadNotes(); });
    }

    setInterval(() => {
        if (notepadLoaded) loadNotepadNotes();
    }, 30000);
