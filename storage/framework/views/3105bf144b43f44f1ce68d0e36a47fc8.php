                                        ...
                                        <?php else: ?>
                                            ...
                                            <td class="text-center sl-sticky-col sl-col-1">
                                                <div class="sl-act-group">
                                                    ...
                                                    <button onclick="window.location.href='<?php echo e($callUrl); ?>'" class="btn btn-warning btn-sm" title="Call">
                                                    ...
                                                    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#delete-<?php echo e($lead->id); ?>" title="Delete">
...
><?php echo e($lead->qa_reason ?? ''); ?></textarea>
                                            ...
                                            <td>
                                                <select class="sl-bubble-select manager-status-dropdown" data-lead-id="<?php echo e($lead->id); ?>" data-current-status="<?php echo e($lead->manager_status ?? 'pending'); ?>">
...
><?php echo e($lead->manager_reason ?? ''); ?></textarea>
                                                <button class="sl-save-btn success save-manager-reason" data-lead-id="<?php echo e($lead->id); ?>">
                                                    <i class="bx bx-save"></i> Save
                                                </button>
                                                <?php if(auth()->user()->hasRole(Roles::SUPER_ADMIN) && $lead->manager_status !== Statuses::MGR_PENDING): ?>
                                                    <button class="sl-save-btn warning reset-manager-status" data-lead-id="<?php echo e($lead->id); ?>" title="Reset to Pending (Super Admin only)">
                                                        <i class="bx bx-undo"></i> Reset
                                                    </button>
                                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                            </td>
                                            <td>
                                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($lead->managerUser): ?>
                                                    <strong style="font-size:.75rem"><?php echo e($lead->managerUser->name); ?></strong>
                                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($lead->manager_reviewed_at): ?>
                                                        <div style="font-size:.63rem;color:#94a3b8;margin-top:1px">
                                                            <?php echo e(\Carbon\Carbon::parse($lead->manager_reviewed_at)->format('M d, h:i A')); ?>
 ...
 <form class="d-inline" action="<?php echo e(route('sales.delete', $lead->id)); ?>" method="POST" >
                                ...
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                    <tr>
                                        <td colspan="<?php echo e(auth()->user()->hasRole(Roles::QA) ? '7' : '19'); ?>" class="text-center" style="padding: 3rem 1rem; color: #94a3b8;">
