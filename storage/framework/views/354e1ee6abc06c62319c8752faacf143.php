...
<div class="ss-hdr">
    ...
    <a href="<?php echo e(route('settings.reports.hub')); ?>" class="ss-back">
...
<div class="ss-filter">
    <form method="GET" action="<?php echo e(route('settings.reports.sales-status')); ?>" style="display:contents">
        <div>
            <label for="ss-from">From</label>
            <input type="date" id="ss-from" name="date_from" value="<?php echo e($dateFrom); ?>">
        </div>
        <div>
            <label for="ss-to">To</label>
            <input type="date" id="ss-to" name="date_to" value="<?php echo e($dateTo); ?>">
        </div>
        <div>
            <label for="ss-field">Date Field</label>
            <select id="ss-field" name="date_field" style="min-width:110px">
                <option value="sale_date" <?php if($dateField === 'sale_date'): echo 'selected'; endif; ?>>Sale Date</option>
                <option value="paid_at"   <?php if($dateField === 'paid_at'): echo 'selected'; endif; ?>>Paid Date</option>
            </select>
        </div>
        <div>
            ...
            <select id="ss-carrier" name="carrier_id" style="min-width:150px">
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $allCarriers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($c->id); ?>" <?php if($carrierId == $c->id): echo 'selected'; endif; ?>><?php echo e($c->name); ?></option>
        ...
        <a href="<?php echo e(route('settings.reports.sales-status')); ?>" class="ss-btn ss-btn-reset" style="align-self:flex-end">
        ...
        <span class="ss-daterange">
            ...
            <span style="opacity:.5;margin-left:.1rem">using <?php echo e($dateField === 'paid_at' ? 'paid date' : 'sale date'); ?></span>
        </span>
...
<div class="ss-kpis">
    <div class="ss-kpi ss-kpi-total">
        ...
        <div class="ss-kpi-val"><?php echo e(number_format($grandTotals['total_sales'] ?? 0)); ?></div>
    ...
    <div class="ss-kpi ss-kpi-issued">
        ...
        <div class="ss-kpi-val"><?php echo e(number_format($grandTotals['issued'] ?? 0)); ?></div>
        <div class="ss-kpi-sub">
            <?php $issPct = ($grandTotals['total_sales']??0) > 0 ? round(($grandTotals['issued']??0) / $grandTotals['total_sales'] * 100, 1) : 0 ?>
            <?php echo e($issPct); ?>% issuance rate
        </div>
    </div>
    <div class="ss-kpi ss-kpi-paid">
        <i class="bx bx-dollar-circle ss-kpi-icon"></i>
        <div class="ss-kpi-lbl">Paid</div>
        <div class="ss-kpi-val"><?php echo e(number_format($grandTotals['paid'] ?? 0)); ?></div>
        <div class="ss-kpi-sub">
            <?php $paidPct = ($grandTotals['total_sales']??0) > 0 ? round(($grandTotals['paid']??0) / $grandTotals['total_sales'] * 100, 1) : 0 ?>
            <?php echo e($paidPct); ?>% payment rate
        </div>
    </div>
    <div class="ss-kpi ss-kpi-nissued">
        <i class="bx bx-x-circle ss-kpi-icon"></i>
        <div class="ss-kpi-lbl">Not Issued</div>
        <div class="ss-kpi-val"><?php echo e(number_format($grandTotals['not_issued'] ?? 0)); ?></div>
        <div class="ss-kpi-sub">
            <?php $niPct = ($grandTotals['total_sales']??0) > 0 ? round(($grandTotals['not_issued']??0) / $grandTotals['total_sales'] * 100, 1) : 0 ?>
            <?php echo e($niPct); ?>% not issued
        </div>
    </div>
</div>
        ...
        <h6>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($carrierId): ?>
                <span style="font-size:.65rem;font-weight:400;color:var(--ss-text-3);margin-left:.3rem">
                    — filtered: <?php echo e($allCarriers->firstWhere('id', $carrierId)?->name ?? 'Selected Carrier'); ?>
        ...
        <table class="ss-tbl">
            <thead>
                <tr>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $stageKeys; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sk): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <th class="<?php echo e($thClasses[$sk]); ?>" style="min-width:68px" title="<?php echo e($stageLabels[$sk]); ?>">
                        <i class="bx <?php echo e($stageIcons[$sk]); ?>" style="font-weight:400"></i>
                        <?php echo e($sk === 'policy_died' ? 'Died/CB' : $stageLabels[$sk]); ?>
            ...
            <tbody>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $carriersData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $rank   = $loop->iteration;
                ...
                <tr>
                    ...
                    <td class="ss-rank" style="text-align:center">
                        <span class="<?php echo e($rank===1?'rank-1':($rank===2?'rank-2':($rank===3?'rank-3':''))); ?>"><?php echo e($rank); ?></span>
                    </td>
                    
                    <td class="td-carrier">
                        <div class="ss-carrier-cell">
                            <span class="ss-dot" style="background:<?php echo e($color); ?>;box-shadow:0 0 0 2px <?php echo e($color); ?>30"></span>
                            <span class="ss-cname" title="<?php echo e($row->carrier_name); ?>"><?php echo e($row->carrier_name); ?></span>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($row->assigned_partner): ?>
                                <span class="ss-partner-badge <?php echo e($badgeClass); ?>"><?php echo e($row->assigned_partner); ?></span>
                            <?php else: ?>
                    ...
                    ?>
                    <td class="<?php echo e($stageCols[$sk]); ?>">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($cnt > 0): ?>
                        <a href="<?php echo e($ddUrl); ?>" class="ss-stage-link" title="<?php echo e($stageLabels[$sk]); ?>: <?php echo e($cnt); ?> leads">
                            <?php echo e(number_format($cnt)); ?>
                    ...
                    <td class="ss-prog-cell">
                        <div class="ss-prog">
                            <div class="ss-prog-track">
                                <div class="ss-prog-fill" style="width:<?php echo e($pct); ?>%;background:<?php echo e($color); ?>"></div>
                            </div>
                            <span class="ss-pct"><?php echo e($pct); ?>%</span>
                        </div>
                    </td>
                </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
<div class="ss-footnote">
    ...
    Date filter applied to <code><?php echo e($dateField === 'paid_at' ? 'paid_at' : 'sale_date'); ?></code>.
    Click any number to drill down to the individual leads behind it.
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/taurus-crm/resources/views/admin/reports/sales-status.blade.php ENDPATH**/ ?>