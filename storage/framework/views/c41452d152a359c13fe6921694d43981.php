...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($unassignedCarriers->count() > 0): ?>
<div style="margin-top:1.2rem">
    <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.6rem;flex-wrap:wrap">
        ...
        <span style="font-size:.62rem;background:rgba(241,180,76,.12);color:#b87a14;border:1px solid rgba(241,180,76,.25);border-radius:999px;padding:.1rem .55rem;font-weight:700">
            <?php echo e($unassignedCarriers->count()); ?> carrier<?php echo e($unassignedCarriers->count() != 1 ? 's' : ''); ?>
    ...
    <div style="background:var(--bs-card-bg);border:1px solid rgba(241,180,76,.2);border-radius:.55rem;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.04)">
        <table style="width:100%;border-collapse:collapse;font-size:.72rem">
            ...
            <tbody>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $unassignedCarriers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $uc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr style="border-bottom:1px solid rgba(0,0,0,.04);transition:background .1s" onmouseover="this.style.background='rgba(241,180,76,.04)'" onmouseout="this.style.background=''">
                    <td style="padding:.5rem .75rem;font-weight:700;color:var(--bs-surface-700)">
                        <i class="bx bx-shield-quarter" style="color:#556ee6;margin-right:.3rem"></i><?php echo e($uc->name); ?>

                    </td>
                    <td style="padding:.5rem .75rem;color:var(--bs-surface-500)"><?php echo e(ucwords(str_replace('_', ' ', $uc->payment_module))); ?></td>
                    <td style="padding:.5rem .75rem;color:var(--bs-surface-500)"><?php echo e($uc->base_commission_percentage ? number_format($uc->base_commission_percentage, 1).'%' : '—'); ?></td>
                    <td style="padding:.5rem .75rem">
                        <span style="font-size:.6rem;padding:.1rem .45rem;border-radius:999px;font-weight:700;<?php echo e($uc->is_active ? 'background:rgba(52,195,143,.12);color:#1a8754;' : 'background:rgba(244,106,106,.1);color:#c84646;'); ?>">
                            <?php echo e($uc->is_active ? 'Active' : 'Inactive'); ?>

                        </span>
                    </td>
                    <td style="padding:.5rem .75rem;text-align:right">
                        <?php if(auth()->check() && auth()->user()->canEditModule('carriers')): ?>
                        <a href="<?php echo e(route('admin.insurance-carriers.edit', $uc->id)); ?>" style="font-size:.62rem;font-weight:600;color:#556ee6;text-decoration:none;padding:.2rem .5rem;border:1px solid rgba(85,110,230,.2);border-radius:.3rem;margin-right:.3rem">
                            <i class="bx bx-edit-alt"></i> Edit
                        </a>
                        <a href="<?php echo e(route('admin.partners.index')); ?>" style="font-size:.62rem;font-weight:600;color:#b87a14;text-decoration:none;padding:.2rem .5rem;border:1px solid rgba(241,180,76,.25);border-radius:.3rem">
