...
<?php $__env->startSection('content'); ?>
 ...
 <form class="d-inline" action="<?php echo e(route('admin.insurance-carriers.destroy', $pc['carrier']->id)); ?>" method="POST" onsubmit="return confirm('⚠️ DANGER: Are you sure you want to PERMANENTLY DELETE the carrier <?php echo e($pc['carrier']->name); ?> from the entire system? This will remove ALL partner assignments and cannot be undone!')">
                                <?php echo csrf_field(); ?>
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(isset($pc['partner']->is_partner_model) && $pc['partner']->is_partner_model): ?>
 <form class="d-inline" action="<?php echo e(route('admin.partners.remove-carrier-assignment', [$pc['partner']->id, $pc['carrier']->id])); ?>" method="POST" onsubmit="return confirm('Are you sure you want to remove <?php echo e($pc['carrier']->name); ?> assignment from partner <?php echo e($pc['partner']->name); ?>? This will remove all state assignments for this carrier.');">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                            ...
                            <?php else: ?>
 <form class="d-inline" action="<?php echo e(route('admin.partners.remove-carrier-assignment', [$pc['partner']->id, $pc['carrier']->id])); ?>" method="POST" onsubmit="return confirm('Are you sure you want to remove <?php echo e($pc['carrier']->name); ?> assignment from agent <?php echo e($pc['partner']->name); ?>? This will remove all state assignments for this carrier.');">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                                    <button type="submit" class="btn btn-sm btn-outline-danger" title="Remove Carrier Assignment">
                                        <i class="mdi mdi-delete me-1"></i>Remove Assignment
